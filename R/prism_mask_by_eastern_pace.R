...
bounds.dewa <- readShapePoly('/mnt/arctic/c/Share/LCC-VP/kguay/bounds/DEWA_park_crs')
bounds.shen <- readShapePoly('/mnt/arctic/c/Share/LCC-VP/kguay/bounds/SHEN_park_crs')
bounds.grsm <- readShapePoly('/mnt/arctic/c/Share/LCC-VP/kguay/bounds/GRSM_park_crs')

# load PRISM file
prism <- prism <- raster('/mnt/arctic/c/Share/LCC-VP/DCP30/seasonal/ea_tasmax_rcp45_2060_seas.tif')
...
bounds.pace <- spTransform(bounds.pace, CRS(projection(prism)))
bounds.dewa <- spTransform(bounds.dewa, CRS(projection(prism)))
bounds.shen <- spTransform(bounds.shen, CRS(projection(prism)))
bounds.grsm <- spTransform(bounds.grsm, CRS(projection(prism)))
